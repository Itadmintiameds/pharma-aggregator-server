package com.example.pharmaaggregatorserver.repository.product;

import com.example.pharmaaggregatorserver.entity.product.CosmeticPersonalCareMasters.ProductsFormMaster;
import com.example.pharmaaggregatorserver.entity.product.ProductFormMaster;
import org.springframework.data.jpa.repository.JpaRepository;

import java.util.Optional;

public interface ProductsFormMasterRepository extends JpaRepository<ProductsFormMaster, Long> {
    Optional<ProductsFormMaster> findByFormNameIgnoreCase(String formName);
}
