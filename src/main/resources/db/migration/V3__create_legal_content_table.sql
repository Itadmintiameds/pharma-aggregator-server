-- Creates tbl_legal_content (backs entity/content/LegalContent.java) and seeds
-- the buyer and seller Terms & Conditions content that currently lives
-- hardcoded in the frontend (TermsStep.tsx / SellerDeclaration.tsx), so both
-- can be edited going forward without a frontend redeploy.
--
-- CREATE TABLE IF NOT EXISTS is used for the same reason as V1: the table is
-- normally auto-created by Hibernate (ddl-auto=update) in dev/test, so this
-- is a no-op there and only the INSERT applies; on a fresh environment this
-- migration creates the table itself.

CREATE TABLE IF NOT EXISTS tbl_legal_content (
    legal_content_id BIGSERIAL PRIMARY KEY,
